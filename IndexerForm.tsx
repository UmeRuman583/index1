...
export default function IndexerForm({ 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const response = await axios.post(`${apiUrl}/api/index`, {
        urls: urlList,
      ...
      const checkStatus = async () => {
        ...
        if (statusResponse.data.status === 'complete') {
          ...
          setStats({
            ...
            failed: statusResponse.data.failed,
            pending: 0
          })
          
          toast.success(`Indexed ${statusResponse.data.successful}/${statusResponse.data.total} URLs!`, {
            id: 'indexing'
          })
...
https://tier1-backlink.com/link"
            className="w-full h-64 px-4 py-3 rounded-lg border border-gray-300 focus:ring-2 focus:ring-purple-500 focus:border-transparent transition-all font-mono text-sm resize-none"
            disabled={isIndexing}
          />
